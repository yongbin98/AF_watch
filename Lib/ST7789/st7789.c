...
void ST7789_Init(st7789 *st7789)
{
	...
	HAL_Delay(50);
	ST7789_DrawFilledRectangle2(st7789, 0, 0, 240, 240, FLIP_RGB(BLACK));
}
...
//	for(int i=0; i<240; i++)
//	{
//		IMG_128x128_instagram2(i)
//	}
	//ST7789_DrawImage(0, 0, 128, 128, (uint16_t *)saber);
	//ST7789_DrawImage(0, 0, 128, 128, (uint16_t *)IMG_128x128_instagram2);
	//HAL_Delay(3000);
}
...
void ST7789_DrawFilledRectangle2(st7789 *st7789, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t color)
{
	...
	for(int i = 0; i<240; i++)
	{
		for(int k = 0; k<240; k++)
		{
			st7789->LCD_buffer[i][k]=color_buffer;
			//memset(&st7789->LCD_buffer[i][k], color_buffer, sizeof(uint16_t));
		}
	}
	ST7789_DrawImage(x, y, w, h, (uint16_t *)st7789->LCD_buffer);
}


void ST7789_DrawImage_Flip(uint16_t x, uint16_t y, uint16_t w, uint16_t h, const uint16_t *data)
{
	uint16_t* temp_arr=malloc(sizeof(uint16_t)* w * h);
	...
	for(int i = 0; i<w*h; i++)
	{
		temp_arr[i] = FLIP_RGB(data[i]);
	}
	
	ST7789_Select();
	ST7789_SetAddressWindow(x, y, x + w - 1, y + h - 1);
	ST7789_WriteData((uint8_t *)temp_arr, sizeof(uint16_t) * w * h);
	ST7789_UnSelect();
	free(temp_arr);
}
...
void ST7789_WriteChar2(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor)
{
	...
	uint16_t* temp_arr=malloc(sizeof(uint16_t)* font.width * font.height);
	
	for (i = 0; i < font.height; i++) {
		b = font.data[(ch - 32) * font.height + i];
		for (j = 0; j < font.width; j++) {
			if ((b << j) & 0x8000) {
				temp_arr[i*font.width+j]=color_buffer;
			}
			else {
				temp_arr[i*font.width+j]=bgcolor_buffer;
			}
		}
	}
	...
	ST7789_Select();
	ST7789_SetAddressWindow(x, y, x + font.width - 1, y + font.height - 1);
	ST7789_WriteData((uint8_t *)temp_arr, sizeof(uint16_t) * font.width * font.height);
	ST7789_UnSelect();
	free(temp_arr);
}
...
{
	uint16_t str_len = strlen(str);
	uint16_t* temp_arr=malloc(sizeof(uint16_t)* font.width * font.height * str_len * fontsize * fontsize);
	uint32_t i, b, j, ch_cnt;
	uint32_t color_buffer = color;
	uint32_t bgcolor_buffer = bgcolor;	
	...
	for (i = 0; i < font.height; i++) {
		...
		{
			b = font.data[(str[ch_cnt] - 32) * font.height + i];
			for(uint8_t size_hight=0; size_hight < fontsize; size_hight++)
			{
				for (j = 0; j < font.width; j++)
				{
					
					for(uint8_t size_width=0; size_width < fontsize; size_width++)
					{
						
						if ((b << j) & 0x8000) {
							temp_arr[(fontsize*i+size_hight)*font.width*str_len*fontsize+ch_cnt*font.width*fontsize+fontsize*j+size_width]=color_buffer;
						}
						else {
							temp_arr[(fontsize*i+size_hight)*font.width*str_len*fontsize+ch_cnt*font.width*fontsize+fontsize*j+size_width]=bgcolor_buffer;
						}
					}
				}
			}
	...
	ST7789_Select();
	ST7789_SetAddressWindow(x, y, x + font.width * str_len * fontsize - 1, y + font.height * fontsize - 1);
	ST7789_WriteData((uint8_t *)temp_arr, sizeof(uint16_t) * font.width * font.height * str_len * fontsize * fontsize);
	ST7789_UnSelect();
	free(temp_arr);
